var fs = require('fs'),
    ...
    { dbUser, dbPwd, dbName } = require('../config/index');
var options = {
    db_user: dbUser,
    db_pwd: dbPwd,    //数据密码
    db_host: '127.0.0.1',
...
DB.prototype.save = function (table_name, fields, callback) {
    if (!fields) {
        if (callbacl) callback({ msg: 'Field is not allowed for null' });
        return;
    }
    var err_num = 0;
    for (var i in fields) {
    ...
    var node_model = this.getConnection(table_name),
        mongooseEntity = new node_model(fields);
...
DB.prototype.update = function (table_name, conditions, update_fields, callback) {
    ...
    var node_model = this.getConnection(table_name);
    node_model.update(conditions, { $set: update_fields }, {
        multi: false
        , upsert: false
    }, function (err, res) {
        if (err) {
...
DB.prototype.updateData = function (table_name, conditions, update_fields, callback) {
    ...
    var node_model = this.getConnection(table_name);
    node_model.findOneAndUpdate(conditions, update_fields, { multi: false, upsert: false }, function (err, res) {
        if (err) {
            if (callback) callback(err);
        } else {
...
DB.prototype.findSkip = function (table_name, conditions, json2, json3, callback) {
    var node_model = this.getConnection(table_name);
    var col = json2;  /*查询的列*/
    //json3={
    //    page，
